    ...
    methods: {
      _getList(loading = true) {
        let data = {
          ...
          status: this.tabList[this.tabIdx].id
        }
        PrizeApi.getList(data, loading).then((res) => {
          this.$loading.hide()
      ...
      modalConfirm() {
        ...
        switch (this.temporaryType) {
          case 'down':
            break
          case 'del':
            this._prizeDel(this.temporaryItem)
            break
        }
      },
      showEditor(item) {
      ...
      async itemDelete(item) {
        ...
        this.$refs.modal.show({msg: '确定删除该券吗？'})
      },
      _prizeDel(item) {
        PrizeApi.setPrizeDel(item.id).then((res) => {
          this.$loading.hide()
          if (res.error === this.$ERR_OK) {
